...
namespace Adapte
{
    public class commonPage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string pathName = sys.RequestPathName();
            DataTable sdr = bllspeciality.GetList("dbo_htmlDir='" + pathName + "'").Tables[0];
            if (sdr.Rows.Count>0)
            {
                articleTitle = Page.Title = sdr.Rows[0]["dbo_name"].ToString();
                PageContent = sdr.Rows[0]["dbo_contenthtml"].ToString();
                nowposition = sdr.Rows[0]["dbo_name"].ToString();
                linknowposition = sys.AppPath + sdr.Rows[0]["dbo_htmlDir"].ToString() + "/";
                fillImg(Convert.ToInt32(sdr.Rows[0]["id"]));
                filltype(Convert.ToInt32(sdr.Rows[0]["id"]));
            }
        }
